...
class Data {
    ...
    set(data, value) {
        if (!fs.existsSync(`nqrdb.json`)) {
            fs.writeFileSync(`nqrdb.json`, "{}");

            if (!data) throw new TypeError("The data is not defined!")
            const file = JSON.parse(fs.readFileSync(`nqrdb.json`, "utf8"))
            file[data] = value
            return fs.writeFileSync(`nqrdb.json`, JSON.stringify(file, null, 1))
        } else {
            if (!data) throw new TypeError("The data is not defined!")
            const file = JSON.parse(fs.readFileSync(`nqrdb.json`, "utf8"))
            file[data] = value
    ...
    fetchAll() {
        const fetchall = JSON.parse(fs.readFileSync("nqrdb.json", "utf8"))
        return fetchall
    }
    delete(data) {
        if (!fs.existsSync(`nqrdb.json`)) {
    ...
    reset() {
        const file = JSON.parse(fs.readFileSync("nqrdb.json", "utf8"))
        return fs.writeFileSync("nqrdb.json", JSON.stringify({}, null, 1))
    }
    all(sınır = 0) {
        if (!fs.existsSync(`nqrdb.json`)) {
            fs.writeFileSync(`nqrdb.json`, "{}");
            const object = JSON.parse(fs.readFileSync("nqrdb.json", "utf8"));
